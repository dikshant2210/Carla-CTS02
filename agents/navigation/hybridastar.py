...
class HybridAStar:
    ...
    def euc_dist(self, position, target, occupancy_grid):
        ...
        return float(output)
        return float(output) + occupancy_grid.static_map[grid_location[0], grid_location[1]]
    ...
    def find_path(self, start, end, occupancy_grid):
        ...
        hq.heappush(open_heap, (cost_to_neighbour_from_start + self.euc_dist(start, end, occupancy_grid), start))

        open_diction[start] = (cost_to_neighbour_from_start + self.euc_dist(start, end, occupancy_grid), start, (start, start))
...
def main():
    ...
    sx, sy, stheta = 2, 234, -90
    gx, gy, gtheta = 2, 152, -90  # 2,4,0 almost exact
    ...
    hy_a_star = HybridAStar(-2, 396, -2, 330, obstacle=obstacle, resolution=1, vehicle_length=2)
    ...
    path = hy_a_star.find_path((sx, sy, stheta), (gx, gy, gtheta), occupancy_grid)
