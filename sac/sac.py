...
from model import GaussianPolicy, QNetwork, SharedNetwork
...
class SAC(object):
    ...
    def select_action(self, state, hidden, evaluate=False):
        ...
        if evaluate is False:
            action, _, _, hidden = self.policy.sample(state, hidden)
        else:
            _, _, action, hidden = self.policy.sample(state, hidden)
