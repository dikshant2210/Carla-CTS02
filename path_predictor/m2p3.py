...
def test(model, num_samples):
    ...
    final_preds = np.zeros((x_batch_test.shape[0], num_samples * test_samples, predicting_frame_num, 2))
    final_gt = np.zeros((x_batch_test.shape[0], num_samples * test_samples, predicting_frame_num, 2))
    ...
    for sample in range(num_samples):
        ...
        # dummy_y = np.zeros((y_batch_test.shape[0], predicting_frame_num, 4)).astype(np.float32)
        dummy_y = np.zeros((y_batch_test.shape[0], predicting_frame_num, 2)).astype(np.float32)
    ...
    if num_samples == 1:
        ...
        final_preds = np.reshape(final_preds, [preds.shape[0], predicting_frame_num, 2])
        final_gt = np.reshape(final_gt, [preds.shape[0], predicting_frame_num, 2])
        with open('final_pred.pkl', 'wb') as file:
            pkl.dump(final_preds, file)
        with open('final_gt.pkl', 'wb') as file:
            pkl.dump(final_gt, file)
        ...
        clustered_preds = np.zeros((y_batch_test.shape[0], clusters, predicting_frame_num, 2))
