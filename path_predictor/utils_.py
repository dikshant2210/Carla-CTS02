...
def calc_mse(bbox_pred, bbox_gt):
    ...
    #for single prediction
        ...
        for i in range(bbox_pred.shape[0]):
            ...
            outputs_x = np.array([bbox_pred[i, :, 0]])
            outputs_x *= rel_x
            # outputs_y = np.array([bbox_pred[i, :, 1] + bbox_pred[i, :, 3]]) / 2
            outputs_y = np.array([bbox_pred[i, :, 1]])
            outputs_y *= rel_y

            targets_x = np.array([bbox_gt[i, :, 0] + bbox_gt[i, :, 0]]) / 2
            # targets_x = np.array([bbox_gt[i, :, 0] + bbox_gt[i, :, 2]]) / 2
            targets_x *= rel_x
            # targets_y = np.array([bbox_gt[i, :, 1] + bbox_gt[i, :, 3]]) / 2
            targets_y = np.array([bbox_gt[i, :, 1] + bbox_gt[i, :, 1]]) / 2
            targets_y *= rel_y
