...
from temp_check.planner.map import CarlaMap
...
class MapHandler(object):
    ...
    def build_map_message(self):
        ...
        # form array for map
        self.map_img = self.carla_map.get_map_lanes()
        # extract green channel, invert, scale to range 0..100, convert to int8
        self.map_img = (100 - self.map_img[..., 1] * 100.0 / 255).astype(np.int8)
...
if __name__ == '__main__':
    mh = MapHandler("Town01")
    print(mh.map_img.shape)
    plt.imshow(mh.map_img, cmap='gray')
    plt.show()
    im = mh.map_img
    im[im[:, :] == 94] = 0.5
